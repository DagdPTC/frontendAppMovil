...
export async function createReserva(payload) {
  if (!payload?.idMesa) throw new Error("Debe seleccionar una mesa.");
  if (!payload?.idTipoReserva) throw new Error("Debe seleccionar un tipo de evento válido.");
  if (!payload?.horaI || !payload?.horaF) throw new Error("Debe especificar el horario completo.");
  if (!payload?.nomCliente?.trim()) throw new Error("El nombre del cliente es obligatorio.");
  if (!payload?.fReserva) throw new Error("La fecha de reserva es obligatoria.");
